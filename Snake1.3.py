...
velocidad_serpiente = 15 #60 fotogramas por segundo.
...
def dibuja_snake(largo, snake_List):
    for coordenada in snake_List:
        pygame.draw.rect(pantalla, azul, [coordenada[0], coordenada[1], largo, largo])
...
def gameLoop():
    ...
    x1=0
    y1=0
    #Declaración de la lista que va a albergar en cada ocasión el cuadro más reciente de la serpiente.
    #Adicionalmente va a tener una variable entera que se incrementa en uno cada vez que se come.
    largo_actual = 1
    ...
    while not game_over:
        ...
        snake_List.append(cabeza_serpiente)

        if len(snake_List) > largo_actual:
            del snake_List[0]

        for element in snake_List[:-1]:
          if element == cabeza_serpiente:
            game_close = True

        dibuja_snake(snake_size, snake_List)

        pygame.draw.rect(pantalla, rojo, [comidaX, comidaY, snake_size, snake_size] ) #Dibuja la comida
        pygame.display.update()
        #Condicional para contabilizar comida y generar nueva.
